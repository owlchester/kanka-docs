...
Dice rolls are templates, attached from an entry, and contain a dice roll value.

Generic dice rolling is possible by writting "d20", "4d4+4", "d%" for percentile and "df" for fudge.

It is also possible to get a character's property by using the `{character.property_name}` syntax. For example, `{character.level}d6+{character.wisdom}`.
