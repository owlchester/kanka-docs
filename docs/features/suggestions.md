...
Various text fields that aren't linked to entries will use display suggestions to re-use the same value. For example the `Type` field on entries, when focused, will display up to **20** (twenty) recently used types on the same category. This means that when you create a character, the type field contains up to 20 recently used types on characters in the campaign.
