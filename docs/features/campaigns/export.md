...
The exported campaign can be one of two types, a `json` export, which contains all the campaign data in `json` format, along with images of entries and the gallery, this kind of export can then be imported into a Kanka campaign, while a `markdown` export, that contains all the entry data in a more human readable `markdown` format, which cannot be imported into a campaign.
